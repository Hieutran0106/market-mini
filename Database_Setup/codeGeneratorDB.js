...
let hoVietNam = ["Nguyễn", "Trần", "Lê", "Phạm", "Huỳnh", "Hoàng", "Phan", "Vũ", "Võ", "Đặng", "Bùi", "Đỗ", "Lý", "Hồ", "Ngô", "Dương"]
let myLocation = JSON.parse(listLocation)
let listTinh = myLocation.map(x => x.path)

let outputStr = "INSERT INTO `khachhang`(`MAKH`, `TENKH`, `GIOITINH`, `SODIENTHOAI`, `DIACHI`, `EMAIL`, `DIEMTHUONG`, `IS_DELETED`) VALUES"
let arrTen = ten.split('\n')
for (let i = 0 ;i<100;i++){
    let hoIndex = Math.floor(Math.random()*hoVietNam.length)
    let arrIndex = Math.floor(Math.random()*arrTen.length)
    let gioiTinh = 0
    if (arrIndex >= 589){
        gioiTinh = 1
    }
    let fullName = hoVietNam[hoIndex]+" "+arrTen[arrIndex]
    let email = arrTen[arrIndex].split(" ").join("") + "@gmail.com"
    email = email.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase()
    let phone = "0" + Math.floor(Math.random()*Math.pow(10,9) + Math.pow(10,8))
    let location = listTinh[Math.floor(Math.random()*listTinh.length)]
    outputStr += `\n\t(null,'${fullName}',${gioiTinh},'${phone}','${location}','${email}',0,0),`
}
