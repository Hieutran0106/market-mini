...
public class HoaDonDAO extends AbtractAccessDatabase<HoaDon> implements ISimpleAccess<HoaDon,Integer> {
    ...
    public boolean insert(HoaDon hoaDon) {
        // Nếu SoVoucher là 0 thì truyền null, nếu khác 0 thì truyền giá trị thực tế
        Object soVoucher = hoaDon.getSoVoucher();
        if (soVoucher instanceof Integer && ((Integer) soVoucher) == 0) {
            soVoucher = null;
        }
        return executeUpdate(
                "INSERT INTO HOADON(NGAYHD,HINHTHUC,TONGTIEN,TIENGIAM,MANV,MAKH,SOVOUCHER,IS_DELETED) VALUES(?,?,?,?,?,?,?,?)",
                hoaDon.getNgayHD(), hoaDon.getHinhThuc(), hoaDon.getTongTien(), hoaDon.getTienGiam(),
                hoaDon.getMaNV(), hoaDon.getMaKH(), soVoucher, hoaDon.isDeleted()
        );
    }
