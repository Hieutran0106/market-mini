...
public class SanPhamAdd extends javax.swing.JFrame {
    ...
    // Đặt mã sản phẩm tiếp theo (chống lỗi khi bảng rỗng)
    private void setNextProductId() {
        SanPham newest = sanphamBUS.getNewest();
        if (newest != null) {
            tfMaSanPham.setText(String.valueOf(newest.getMaSP() + 1));
        } else {
            tfMaSanPham.setText("1");
        }
    }
    ...
    private void button2MouseClicked(java.awt.event.MouseEvent evt) {
        ...
        int maNhaCC = cbNCC.getSelectedIndex() + 1; // Đảm bảo đúng mapping với MANCC
    ...
    private void imagePanel2MouseClicked(java.awt.event.MouseEvent evt) {
